...
class Product(db.Model, BaseMixin):
    ...
    # attributes = db.relationship("Attribute", secondary="product_attribute", back_populates="products")
    image_url = db.Column(db.String(255), nullable=False, name="image_url")
    # size = db.Column(db.Enum(SizeEnum), nullable=True, name="size")
    # color = db.Column(db.Enum(ColorEnum), nullable=True, name="color")
...
class ProductFilterByAttributePrediction(ProductFilterStrategy):
    ...
    def filter(self, products):
        ...
        sorted_result = sorted(filter_result, key=lambda p: p.attributes_prediciton.count(self.attributes_prediction), reverse=True)
