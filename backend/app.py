...
def chatbot():
    ...
    if (compare_strings(response_chatbot, ResponseMessage.MESSAGE_SORRY.value) == True):
        # Lấy ý niệm trước đó
        if len(category_names) > 0:
            ...
            if len(categories_filtered) > 0:
                ...
                response_chatbot = ResponseMessage.MESSAGE_PRO_NOTFOUND.value
                return jsonify({
                    "answer": response_chatbot
                })
        else:
